...
#### Quick install
```shell
sudo apt update && sudo apt install software-properties-common && sudo add-apt-repository --yes --update ppa:ansible/ansible && sudo apt install -y ansible git && cd ~ && git clone https://github.com/CoderXXL/desktop-configurator.git ./.desktop-configurator && cd .desktop-configurator && sudo chmod a+x ./.desktop-configurator/user-handling/create.sh && ansible-galaxy install -r requirements.yaml && ansible-playbook -i inventory.yaml playbook.yaml -K
```
